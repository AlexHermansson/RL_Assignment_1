...
class Environment:
    ...
    def __init__(self, t=15, transition_prob=None, simulation=False):
        self.T = t
        self.p = Position(1, 1)
        self.m = Position(5, 5)
        self.G = Position(5, 5)
    ...
    def _transition_probability(self, next_state, state, action):
        ...
        if not self.simulation:
            ...
            if next_state.done:
                ...
                new_position = state.player.take_action(action, allowed_actions_player)
                if new_position == next_state.player and state.minotaur.manhattan(next_state.minotaur) <= 1:
                    return (1-0.033) / num_allowed_minotaur
...
def PI(environment, lambd=0.967):

    all_states = np.asarray(environment.states)
    all_actions = environment.valid_actions
    policy_0=np.zeros(len(all_states),dtype=int)
    policy_1=np.zeros(len(all_states),dtype=int)
    V_0 = np.zeros(len(all_states))

    for s, state in enumerate(all_states):
        V_0[s] = environment.reward(state)
    iters=0
    while(1):
        iters+=1
        #policy evaluation
        V_1 = np.zeros(len(all_states))
        for s, state in enumerate(all_states):
            policy_action=all_actions[policy_0[s]]
            for next_state in state.actions[policy_action]:
                prob=next_state[1]
        ...
        for s, state in enumerate(all_states):
            ...
            policy_1[s]= np.argmax(action_values)

        if np.array_equal(policy_1,policy_0):
            print('PI converged after',iters,'iterations')
            return policy_1
        else:
            policy_0=np.copy(policy_1)
...
def visualize_policy(opt_policy, minotaur, env):
...
def simulate_game(environment, initial_state, policy, n_games=10000):
    win=0
    maze_exit=Position(5,5)
    all_states = np.asarray(environment.states)
    all_actions = environment.valid_actions
    for game in range(n_games):
        state = initial_state
        state_idx = np.argwhere(all_states == state)[0][0]
        while(1):
            policy_action = all_actions[policy[state_idx]]
            next_states=state.actions[policy_action]
            probs=np.array([])
            next_states_idx=np.array([],dtype=int)
            for next_state in next_states:
                next_states_idx=np.append(next_states_idx,next_state[0])
                probs=np.append(probs,next_state[1])
            state_idx=np.random.choice(next_states_idx,p=probs)
            state=all_states[state_idx]

            if state.player==maze_exit and state.minotaur!=maze_exit:
                win+=1
                break
            elif state.done:
                break

    print(win, 'games won over', n_games)
...
if __name__ == '__main__':
    ...
    #save_obj(env, 'env')
    env = load_obj('env')
    optimal_policy=PI(env)

    #simul_env=Environment(simulation=True)
    #save_obj(simul_env, 'simul_env')
    simul_env=load_obj('simul_env')
    minotaur = Position(3, 4)
    timestep = 15
    #visualize_policy(optimal_policy, minotaur, env)
    all_states=np.asarray(simul_env.states)
    simulate_game(simul_env,all_states[25],optimal_policy)
    # print(V[25])
    # plot_prob(T, V[25])
